...
export default function AISuggestionResult({ open, onClose, samples, statistics, formData, onApplySamples }: AISuggestionResultProps) {
    ...
    if (Array.isArray(answer.answer)) {
      return (
        <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
          ...
          {answer.answer.length > 3 && <Chip label={`+${answer.answer.length - 3}`} size="small" variant="outlined" />}
        </Stack>
      );
    }
  ...
  const renderStatistics = () => (
    <Stack spacing={3}>
      <Alert severity="info">
        <Typography variant="body2">Thống kê được tính toán dựa trên {statistics.totalSamples} mẫu dữ liệu được tạo</Typography>
      </Alert>
                ...
                {stats.mean !== undefined && (
                  <Box>
                    ...
                    <Stack direction="row" spacing={2} sx={{ mt: 1 }}>
                      <Chip label={`Trung bình: ${stats.mean.toFixed(2)}`} size="small" variant="outlined" />
                      {stats.standardDeviation !== undefined && (
                        <Chip label={`Độ lệch chuẩn: ${stats.standardDeviation.toFixed(2)}`} size="small" variant="outlined" />
                      )}
                    </Stack>
                  </Box>
                )}
                  ...
                  <Box>
                    ...
                    <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                      {Object.entries(stats.correlation).map(([otherQuestionId, correlation]) => {
                        ...
                        const color = Math.abs(correlation) > 0.7 ? 'error' : Math.abs(correlation) > 0.3 ? 'warning' : 'default';
                        return (
                          <Chip
                            key={otherQuestionId}
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
                    ...
                    {paginatedSamples.map((sample, index) => (
                      <TableRow key={sample.sampleId} hover>
                        ...
                        {formData.questions.map((question) => {
                          ...
                          return <TableCell key={question.id}>{renderAnswer(answer)}</TableCell>;
                        })}
                      </TableRow>
                    ))}
                  </TableBody>
      ...
      <DialogActions>
        ...
        <Button onClick={handleApply} variant="contained" color="primary" disabled={selectedSamples.size === 0}>
